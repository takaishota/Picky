...
class waplAdminBuilder
{
	...
	var $schema = array(
		...
		'externalImage' => array(
			...
			'children' => array('url', 'transcol', 'safe_mode', 'alt')
		),
		'form' => array(
	...
	function format_text($content, $imagescale = 95, $imagequality = 90, $class = '', $length = null, $transcol = '')
	{
				...
				foreach($element->find('input, textarea, select') as $input)
				{
					...
					if($input->type != 'hidden' && $input->type != 'submit')
					{
						if($input->prev_sibling())
						{
							...
							{
								$text .= '<label>'.$sibling->innertext.'</label>';
							} else if($siblingBackup = $sibling->prev_sibling())
							{
								if($siblingBackup->tag == 'label')
								{
									$text .= '<label>'.$siblingBackup->innertext.'</label>';
								}
							}
						} else
						{
							$text .= '<label>'.str_replace($input->parent, '', $input->parent->parent).'</label>';
						}
					}
					...
					if($input->tag == 'select')
					{
						...
						{
							$text .= '<possibility>';
							$text .= '<label>'.$option->innertext.'</label>';
		...
		{
			$replacements[trim($element->outertext())] = '</quick_text></wordsChunk><wordsChunk class="'.$class.'"><quick_text>'.$element->innertext.'</quick_text></wordsChunk><wordsChunk class="'.$class.'"><quick_text>';
		}
		...
		foreach($html->find('caption') as $element)
		{
			...
			foreach($element->find('img') as $img)
			{
				...
				$text .= '<row><cell class="postCaptionImg"><externalImage filetype="'.strtolower(substr($img->src,-3)).'" scale="'.$imagescale.'" quality="'.$imagequality.'"><url>'.htmlspecialchars($img->src).'</url>'.$transcoltext.'</externalImage></cell></row>';
			}
		...
		foreach($html->find('img') as $element)
		{
			...
			// don't scale smiley images
			$noscaleimageclasses = array('wp-smiley', 'sociable-hovers');
			if(in_array($element->class, $noscaleimageclasses))
			{
				$thisimagescale = 0;
			} else
			{
			...
			if($element->parent->class == 'share' || $element->parent->parent->class == 'share' || $element->parent->parent->parent->class == 'share')
			{
				$thisimagescale = 0;
			}
			...
			$innerText = '<externalImage filetype="'.strtolower(substr($element->src,-3)).'" scale="'.$thisimagescale.'" quality="'.$imagequality.'"><url>'.htmlspecialchars($element->src).'</url>'.$transcoltext.'<alt>"'.$alt.'"</alt></externalImage>';
			...
			$replacements[trim($element->outertext())] = $text;
		}
		...
		foreach($html->find('span') as $element)
		{
			...
			{
				$replacements[trim($element->outertext())] = '[b]'.$element->innertext.'[/b]';
			} else if($element->style == 'font-style: italic;')
			{
				$replacements[trim($element->outertext())] = '[i]'.$element->innertext.'[/i]';
			} else if($element->style == 'text-decoration: underline;')
			{
				$replacements[trim($element->outertext())] = '[u]'.$element->innertext.'[/u]';
			} else if($element->style == 'font-weight: bold; font-style: italic;')
			{
				$replacements[trim($element->outertext())] = '[i][b]'.$element->innertext.'[/b][/i]';
			} else if($element->style == 'text-decoration: line-through;')
			{
				$replacements[trim($element->outertext())] = '[s]'.$element->innertext.'[/s]';
			}
		}
		...
		{
			$replacements[trim($element->outertext())] = '</quick_text></wordsChunk><wordsChunk class="'.$class.'"><display_as>'.$element->tag.'</display_as><quick_text>'.$element->innertext.'</quick_text></wordsChunk><wordsChunk class="'.$class.'"><quick_text>';
		}
		...
		foreach($html->find('a') as $element)
		{
			...
			{
				$replacements[trim($element->outertext())] = '[url='.htmlspecialchars($element->href).']'.$element->innertext.'[/url]';
			}
		}
		...
		$stripTagExcludeString.= '<row><cell>';
		...
		$empty = wpma_str_get_html($content);
		...
		if($length)
		{
			...
			$bbhtml = wpma_str_get_html($tidyContent);
